...
func TestSetNodeOperatorAdded(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, event, db.Operators[operatorID].NodeOperatorEvents.NodeOperatorAdded)
}
...
func TestGetNodeOperatorAdded(t *testing.T) {
	...
	event, err := storageAdapter.GetNodeOperatorAdded(operatorID)
	assert.NoError(t, err)

	expectedEvent := initialData.Operators[operatorID].NodeOperatorEvents.NodeOperatorAdded
...
func TestSetNodeOperatorManagerAddressChanged(t *testing.T) {
	...
	db, err := storageAdapter.LoadDatabase()
	assert.NoError(t, err)
	assert.Equal(t, event, db.Operators[operatorID].NodeOperatorEvents.NodeOperatorManagerAddressChanged)
}
...
func TestGetNodeOperatorManagerAddressChanged(t *testing.T) {
	...
	event, err := storageAdapter.GetNodeOperatorManagerAddressChanged(operatorID)
	assert.NoError(t, err)

	expectedEvent := initialData.Operators[operatorID].NodeOperatorEvents.NodeOperatorManagerAddressChanged
	assert.Equal(t, expectedEvent, event)
}
...
func TestSetNodeOperatorRewardAddressChanged(t *testing.T) {
	...
	db, err := storageAdapter.LoadDatabase()
	assert.NoError(t, err)
	assert.Equal(t, event, db.Operators[operatorID].NodeOperatorEvents.NodeOperatorRewardAddressChanged)
}
...
func TestGetNodeOperatorRewardAddressChanged(t *testing.T) {
	...
	event, err := storageAdapter.GetNodeOperatorRewardAddressChanged(operatorID)
	assert.NoError(t, err)

	expectedEvent := initialData.Operators[operatorID].NodeOperatorEvents.NodeOperatorRewardAddressChanged
	assert.Equal(t, expectedEvent, event)
}
